...
#import "LTNJSONRequestSerializer.h"
#import "LTNQueryParamCollection.h"
#import "LTNDefaultConfiguration.h"

NSString *const LTNResponseObjectErrorKey = @"LTNResponseObject";

static NSString * const kLTNContentDispositionKey = @"Content-Disposition";

static NSDictionary * LTN__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * LTN__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = LTN__headerFieldsForResponse(response);
    if(!headers[kLTNContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kLTNContentDispositionKey];
...
@interface LTNApiClient ()

@property (nonatomic, strong, readwrite) id<LTNConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation LTNApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static LTNApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[LTNDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[LTNDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithConfiguration:(id<LTNConfiguration>)configuration {
    return [self initWithBaseURL:[NSURL URLWithString:configuration.host] configuration:configuration];
}

- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<LTNConfiguration>)configuration {
    self = [super initWithBaseURL:url];
    if (self) {
        _configuration = configuration;
        _timeoutInterval = 60;
        _responseDeserializer = [[LTNResponseDeserializer alloc] init];
        _sanitizer = [[LTNSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        LTNJSONRequestSerializer * swgjsonRequestSerializer = [LTNJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kLTNApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        LTNDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[LTNResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = LTN__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<LTNSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = LTNPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = LTNPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, LTNPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[LTNQueryParamCollection class]]){
            LTNQueryParamCollection * coll = (LTNQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = LTNPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = LTNPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = LTNPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)queries WithAuthSettings:(NSArray *)authSettings {
    ...
    id<LTNConfiguration> config = self.configuration;
